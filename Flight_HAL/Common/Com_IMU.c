...
void GetAngle(const _stMPU *pMpu, _stAngle *pAngE, float dt)
{
    ...
    // 角速度融合加速度积分补偿值
    Gyro.x = pMpu->gryoX * Gyro_Gr + KpDef * AccGravity.x + GyroIntegError.x; // 弧度制
    Gyro.y = pMpu->gryoY * Gyro_Gr + KpDef * AccGravity.y + GyroIntegError.y;
    Gyro.z = pMpu->gryoZ * Gyro_Gr + KpDef * AccGravity.z + GyroIntegError.z;
...
#else
        float yaw_G = pMpu->gryoZ * Gyro_G;    // 将Z轴角速度陀螺仪值 转换为Z角度/秒      Gyro_G陀螺仪初始化量程+-2000度每秒于1 / (65536 / 4000) = 0.03051756*2
